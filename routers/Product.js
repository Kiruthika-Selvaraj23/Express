...
ProductRouter.post("/product", Auth , async (req, res) => {
    try {
...
ProductRouter.put("/updateProduct/:id", Auth, async (req, res) => {
    try {
...
ProductRouter.delete("/deleteProduct/:id", Auth, async (req, res) => {
    try {
        const id = req.params.id
        if (!id) {
            return res.send({ success: false, message: "Id is missing" })
